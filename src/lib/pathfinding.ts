...
const customPointNodes = new Map(
  MAP_DATA.POINTS.filter((point) => !roomIds.has(point.id)).map((point) => [
    point.id,
    {
      id: point.id,
      x: point.x,
...
const availableFloors = Array.from(
  new Set(
    MAP_DATA.ROOMS.filter((room) => !isStairRoomId(room.id)).map(room => room.floor)
  )
).sort((a, b) => a - b);
